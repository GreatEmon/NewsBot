...
export default class News extends Component {
      ...
      constructor(){
        ...
        this.state = {
            ...
            page : 1,
            totalResults :thisarticles.totalResults,
            loading :false
        }
      }
    async componentDidMount(){
        let url =`https://newsapi.org/v2/top-headlines?country=us&category=business&apiKey=c141dbcf8e8b4df99a231845da9c544a&page=1&pageSize=${this.props.pageSize}`;
        let data =await fetch(url);
        let parsedata =await data.json();
        this.setState({articles : parsedata.articles})
    }
    handlePrev =async ()=>{
        let url =`https://newsapi.org/v2/top-headlines?country=us&category=business&apiKey=c141dbcf8e8b4df99a231845da9c544aa&page=${this.state.page - 1}&pageSize=${this.props.pageSize}`;
        let data =await fetch(url);
        let parsedata =await data.json();
        this.setState({
    ...
    handleNext =async ()=>{
        ...
        let url =`https://newsapi.org/v2/top-headlines?country=us&category=business&apiKey=c141dbcf8e8b4df99a231845da9c544a&page=${this.state.page + 1}&pageSize=${this.props.pageSize}`;
        let data =await fetch(url);
        let parsedata =await data.json();
        this.setState({
            page: this.state.page + 1,
  ...
  render() {
    ...
    return (
      ...
      <div className="container d-flex justify-content-between pb-4">
        ...
        <button  type="button" className="btn btn-primary btn" onClick={this.handleNext} id="nextbtn">Next&rarr;</button>
      </div>
      </>
    )
  }
