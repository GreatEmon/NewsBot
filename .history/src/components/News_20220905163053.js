...
export default class News extends Component {
    ...
    async componentDidMount(){
     let url ="https://newsapi.org/v2/everything?q=tesla&from=2022-08-05&sortBy=publishedAt&apiKey=c141dbcf8e8b4df99a231845da9c544a&page=1&pa";
     let data =await fetch(url);
     let parsedata =await data.json();
     this.setState({articles : parsedata.articles})
    }
    handlePrev =async ()=>{
        let url =`https://newsapi.org/v2/everything?q=tesla&from=2022-08-05&sortBy=publishedAt&apiKey=c141dbcf8e8b4df99a231845da9c544a&page=${this.state.page - 1}&pa`;
        let data =await fetch(url);
        let parsedata =await data.json();
        this.setState({
